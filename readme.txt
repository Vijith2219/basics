...
mobility in a city.
Combine historical usage patterns with
weather data in order to forecast hourly bike rental demand.

The repo includes train.csv, test.csv, model.ipynb and 'submission.csv'
train.csv to train the model and it includes data such as datetime, season, 
holiday,working day, weather, temp, humidity, casual, registered and the target
variable count. 

predictions are evaluated using RMSLE (Root Mean Squared Logarithmic Error)